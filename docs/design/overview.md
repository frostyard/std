...
- `JSONReporter` never drops an event because its `Details` cannot be encoded: when `Complete` (the only method that forwards arbitrary caller data) receives a value `encoding/json` rejects (func, channel, NaN/Inf, failing or panicking `Marshaler`), the event is re-emitted with `Details` set to `{"encoding_error": "<reason>"}` so consumers still see the terminal `complete` line. A panic raised while marshaling `Details` is recovered rather than propagated to the caller. Writer (I/O) errors stay silent to callers but make that reporter fail-stop: every later event is dropped so no valid-looking record can follow a partial JSON fragment — see [reporter-package.md](../specs/reporter-package.md#jsonreporter-reporterjsongo)
