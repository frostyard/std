...
- [ADR-0038 — make ci stays canonical; the TestI/Integration name filter is chairlift-only](https://github.com/frostyard/core/blob/main/docs/adr/0038-scope-the-test-name-filter-to-chairlift.md) — `make ci` is std's canonical credential-free gate, running its own checks sequentially and fail-fast (tidy diff, vet, gofmt, lint at the pin, unit+e2e tests, race, cross-arch `_examples/` build); it is not a mirror of `.github/workflows/ci.yml`, whose Security Scan, Docs integrity, and Release config jobs run only on GitHub, while the cross-arch `_examples/` build runs in both — `make ci` and the workflow's Examples Cross-Arch Build job (`examples-build`) enumerate the same programs with `scripts/example-dirs.sh` and build the same `linux/amd64`/`linux/arm64` pair. std is not chairlift, so no test-name filter applies and every hermetic test runs in the gate and in pull-request CI
