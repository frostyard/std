...
- **Panic recovery:** `emit()` encodes through a `safeEncode` helper that recovers a panic raised while marshaling the event — for example a caller-supplied `Details` value whose `MarshalJSON` panics instead of returning an error — and wraps it as an `encodingPanicError`, matched by `isEncodingError`'s `errors.As` check alongside the `json.Unsupported*Error`/`json.MarshalerError` cases above. A recovered panic therefore takes the same `encoding_error` fallback path as any other encode failure, so a hostile or buggy `Details` value cannot crash the caller
